...
module.exports = ({ strapi }) => ({
    ...
    async getOktabitData(entry, categoryMap) {
        try {
            ...
            const response = Axios.get(`https://www.oktabit.gr/api/data/?format=json`,
                {
                    headers: {
            ...
            const availableProducts = this.filterData(data, categoryMap)

            return { products: availableProducts, downloadingAllSuccess }
        } catch (error) {
            return { message: "Error" }
        }
    },

    // async getOktabitData1(entry, categoryMap) {
    //     try {
    //         let downloadingAllSuccess = true
    //         const { categories_map, char_name_map, char_value_map, stock_map,
    //             isWhitelistSelected, whitelist_map, blacklist_map,
    ...
    filterData(data, categoryMap) {
        ...
        function filterCategories(cat) {
            ...
            if (categoryMap.isWhitelistSelected) {
                if (categoryMap.whitelist_map.length > 0) {
                    let catIndex = categoryMap.whitelist_map.findIndex(x => x.name.trim() === cat.parent_category.trim())
                    if (catIndex !== -1) {
                        if (categoryMap.whitelist_map[catIndex].subcategory.length > 0) {
                            let subIndex = categoryMap.whitelist_map[catIndex].subcategory.findIndex(x => x.name.trim() === cat.subcategory.trim())
                            if (subIndex !== -1) {
                                true
                            }
                            else {
                ...
                if (categoryMap.blacklist_map.length > 0) {
                    let catIndex = categoryMap.blacklist_map.findIndex(x => x.name.trim() === cat.parent_category.trim())
                    if (catIndex !== -1) {
                        if (categoryMap.blacklist_map[catIndex].subcategory.length > 0) {
                            let subIndex = categoryMap.blacklist_map[catIndex].subcategory.findIndex(x => x.name.trim() === cat.subcategory.trim())
                            if (subIndex !== -1) {
                                return false
                            }
                            else {
        ...
        function filterPriceRange(priceRange) {
            ...
            const productPrice = parseFloat(priceRange.timi_xontrikis)
