...
const MappingCharacteristics = ({ characteristic }) => {
    ...
    const updateCharNameMapping = (id) => {
        let new_map = importMapping.char_name_map.map((item) => {
            if (item.id === id) {
                item.name = name,
                    item.value = value
            }
        ...
        setImportMapping({ ...importMapping, char_name_map: new_map })
    }

    const handleDeleteCharName = () => {
        let new_map = importMapping.char_name_map.filter(item => item.id !== characteristic.id)
    ...
    return (
        <Tr>
            ...
            <Td>
                <IconButton noBorder onClick={() => handleDeleteCharName()} label="Delete" icon={<Trash />} />
            </Td>
        </Tr >
    )
}

const MappingCharName = ({ mapping }) => {
    const [importMapping, setImportMapping] = useContext(MappingContext)
    ...
    const handleAddCharTitle = () => {
        ...
        setNewID(newID + 1)
        let new_char_name_map = [...importMapping.char_name_map, new_map]
        setImportMapping({ ...importMapping, char_name_map: new_char_name_map })
    }
...
export default MappingCharName